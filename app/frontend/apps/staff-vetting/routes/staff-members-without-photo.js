...
import { getStaffMembersWithoutPhoto } from '../requests';

class StaffMembersWithoutPhoto extends Component {
  state = {
  ...
  componentDidMount() {
    if (this.props.count > 0) {
      getStaffMembersWithoutPhoto().then(res => {
        const imStaffMembers = Immutable.fromJS(
          res.data.staffWithoutPhoto.map(staffMember => ({
            ...staffMember,
...
StaffMembersWithoutPhoto.propTypes = {
...
export default StaffMembersWithoutPhoto;
