...
import { getStaffMembersWithoutAddress } from '../requests';

class StaffWithoutAdress extends Component {
  state = {
  ...
  componentDidMount() {
    if (this.props.count > 0) {
      getStaffMembersWithoutAddress().then(res => {
        const imStaffMembers = Immutable.fromJS(
          res.data.staffWithoutAddress.map(staffMember => ({
            ...staffMember,
...
export default StaffWithoutAdress;
