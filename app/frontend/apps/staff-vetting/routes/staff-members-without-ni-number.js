...
import { getStaffMembersWithoutNINumber } from '../requests';

class StaffMembersWithoutNINumber extends Component {
  state = {
  ...
  componentDidMount() {
    if (this.props.count > 0) {
      getStaffMembersWithoutNINumber().then(res => {
        const imStaffMembers = Immutable.fromJS(
          res.data.staffWithoutNiNumber.map(staffMember => ({
            ...staffMember,
...
StaffMembersWithoutNINumber.propTypes = {
...
export default StaffMembersWithoutNINumber;
