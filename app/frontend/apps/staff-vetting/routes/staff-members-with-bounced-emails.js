...
import { getStaffMembersWithBouncedEmails } from '../requests';
...
class StaffWithBouncedEmails extends Component {
  ...
  componentDidMount() {
    if (this.props.count > 0) {
      getStaffMembersWithBouncedEmails().then(res => {
        const imStaffMembers = Immutable.fromJS(
          res.data.staffMembersWithBouncedEmail.map(staffMember => ({
            ...staffMember,
  ...
  render() {
    ...
    return (
      <Page
        ...
        tabsFilterRenderer={() => <TabFilter showSecurity showVenues />}
        staffMemberListRenderer={staffMembers => (
          <StaffMemberList staffMembers={staffMembers} withBouncedEmail />
        )}
      />
    );
  }
