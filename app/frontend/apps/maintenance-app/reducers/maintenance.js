...
const maintenance = (state = initialState, action) => {
  ...
  case constants.INITIAL_LOAD:
    return state.set(
      ...
      'priorities', action.initialData.priorities
    ).set(
      'statuses', action.initialData.statuses
    ).set(
      'maintenanceTasks', action.initialData.maintenanceTasks
    ).set(
      'currentUser', action.initialData.currentUser
    ).set(
      'filter', Object.assign({}, state.get('filter'), { totalCount: window.boss.maintenance.totalCount, totalPages: window.boss.maintenance.totalPages })
    );
