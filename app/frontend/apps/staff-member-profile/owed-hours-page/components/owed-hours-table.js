...
export const getOwedHourUIData = owedHourJS => {
  ...
  let times = 'N/A';
  if (hasDate) {
    const startTime = safeMoment
      .iso8601Parse(oFetch(owedHourJS, 'times.startsAt'))
      .utcOffset(oFetch(owedHourJS, 'times.startsAt'))
      .format('HH:mm');
    const endTime = safeMoment
      .iso8601Parse(oFetch(owedHourJS, 'times.endsAt'))
      .utcOffset(oFetch(owedHourJS, 'times.endsAt'))
      .format('HH:mm');
...
const OwedHoursTable = props => {
  const [owedHours, deleteOwedHours, openEditModal, isStaffMemberDisabled, owedHoursPermissions] = oFetch(
    props,
    'owedHours',
    'deleteOwedHours',
    'openEditModal',
