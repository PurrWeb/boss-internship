...
class VenueShifts extends Component {
  ...
  renderShiftsRotaed(rotaShifts) {
    ...
    return (
      <p className="boss-timeline__text">
        <span className="boss-timeline__text-faded">Rotaed:</span>{' '}
        {rotaedList.size === 0 ? 'N/A' : rotaedList.join(', ')}
      </p>
    );
  }
  ...
  render() {
    ...
    const allBreaks = hoursAcceptancePeriods.reduce((acc, hoursAcceptancePeriod) => {
      const breaks = oFetch(hoursAcceptancePeriod, 'breaks');
      if (breaks.size === 0) {
        return acc;
      }
      const breaksDiff = utils.getStartsEndsTimeDiff(breaks);
      return acc + breaksDiff;
    }, 0);
    return (
            ...
            <div className="boss-timeline__details-header">
              <div>
                ...
                {!isSecurityVenue &&
                  allBreaks !== 0 && (
                    <p className="boss-timeline__text boss-timeline__text_role_hours">
                      <span className="boss-timeline__text-marked">{utils.formattedTime(allBreaks)} </span>
                      <span>breaks</span>
                    </p>
                  )}
              </div>
              <div
                ...
                className={`boss-timeline__details-switch ${
                  isOpen ? '' : 'boss-timeline__details-switch_state_closed'
                }`}
              >
                Toggle details
              </div>
            </div>
            <Collapse isOpened={isOpen} className={`boss-timeline__details-content`}>
              <div className="boss-timeline__details-inner">
                {this.renderShiftsRotaed(rotaShifts)}
                {!hasHoursAcceptancePeriods && (
                  <a
                    href={isSecurityVenue ? appRoutes.securityRotaDaily(date) : appRoutes.rotaDaily(date, venueId)}
                    target="_blank"
              ...
              <div className="boss-timeline__details-inner">
                ...
                {hasHoursAcceptancePeriods && (
                  <a
                    href={appRoutes.staffMemberHoursOverview(staffMemberId, date)}
                    target="_blank"
                    className="boss-timeline__link boss-timeline__link_role_details"
                  >
                    View Details
