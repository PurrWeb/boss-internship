...
class HolidayItem extends React.Component {
  ...
  render() {
    ...
    return (
      <p className="boss-summary__text">{date}</p>
    )
  }
}
...
class RotaShiftItem extends React.Component {
  ...
  holidayDate = (startsAt, endsAt) => {
    return `${safeMoment.iso8601Parse(startsAt).format('DD MMM HH:mm')} to ${safeMoment.iso8601Parse(endsAt).format('HH:mm')}`;
  }
  
  render() {
    const rotaShift = this.props.item;
    const startsAt = rotaShift.get('starts_at');
    const endsAt = rotaShift.get('ends_at');
        ...
        <div className="boss-summary">
          <div className="boss-summary__header">
            <p className="boss-summary__text">
              <span className="boss-summary__text-marked">{count}{suffix}</span> {text}
            </p>
          </div>
...
export default class StaffMemberItem extends React.Component {
  ...
  render() {
    ...
    const avatar = staffMember.get('avatar_url');
    const fullName = `${staffMember.get('first_name')} ${staffMember.get('surname')}`
    const staffType = this.props.staffTypes.find(staffType => staffType.get('id') === staffMember.get('staff_type'));
    ...
    return (
      <div className="boss-staff-summary__row">
        ...
        <div className="boss-staff-summary__cell boss-staff-summary__cell_role_action">
          ...
          {(this.props.currentStaffId === staffId) && <div ref={this.props.setRef} className="boss-popover boss-popover_context_rotas-daily boss-popover_state_opened">
