...
import {
  ...
  undoAcceptedAccessoryRequestRequest,
  undoRejectedAccessoryRequestRequest
} from '../requests';
...
export const loadInitialData = () => (dispatch, getState) => {
  ...
  return loadInitialDataRequest({venueId: currentVenue, currentPage: currentPage})
    .then(resp => {
...
export const acceptAccessoryRequest = ({requestId, accessoryId}) => (dispatch, getState) => {
  const currentVenue = getState().getIn(['accessoryRequestsPage', 'currentVenue']);
  return acceptAccessoryRequestRequest({venueId: currentVenue, accessoryId, requestId})
    .then(resp => {
...
export const rejectAccessoryRequest = ({requestId, accessoryId}) => (dispatch, getState) => {
  const currentVenue = getState().getIn(['accessoryRequestsPage', 'currentVenue']);
  return rejectAccessoryRequestRequest({venueId: currentVenue, accessoryId, requestId})
    .then(resp => {
      dispatch(updateRequestInStore(resp.data));
      return resp;
    });
...
export const undoAcceptedAccessoryRequest = ({requestId, accessoryId}) => (dispatch, getState) => {
  const currentVenue = getState().getIn(['accessoryRequestsPage', 'currentVenue']);
  return undoAcceptedAccessoryRequestRequest({venueId: currentVenue, accessoryId, requestId})
    .then(resp => {
      dispatch(updateRequestInStore(resp.data));
      return resp;
    });
...
export const undoRejectedAccessoryRequest = ({requestId, accessoryId}) => (dispatch, getState) => {
  const currentVenue = getState().getIn(['accessoryRequestsPage', 'currentVenue']);
  return undoRejectedAccessoryRequestRequest({venueId: currentVenue, accessoryId, requestId})
    .then(resp => {
      dispatch(updateRequestInStore(resp.data));
      return resp;
    });
