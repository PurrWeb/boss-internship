...
export default class MessageCarousel extends React.Component {
  constructor(props) {
    ...
    this.state = {
      currentMessage: window.boss.venueDashboard.messages[0],
    }
  }
  ...
  renderPreviousButton() {
    if (this.lastMessage(this.state.currentMessage)) {
      ...
      return (
        <button className="boss-button boss-button_role_previous boss-message__meta-action" onClick={ this.handlePreviousClick.bind(this) }>See Previous</button>
      );
    }
  }
  ...
  renderNextButton() {
    if (this.firstMessage(this.state.currentMessage)) {
      ...
      return (
        <button className="boss-button boss-button_role_next boss-message__meta-action" onClick={ this.handleNextClick.bind(this) }>See Next</button>
      );
    }
  }

  render() {
    if (!this.props.messages.length) return <span></span>;
    ...
    return (
      <div className="boss-message boss-message_role_dashboard boss-message_type_accent boss-message_context_board">
        ...
        <div className="boss-message__content" dangerouslySetInnerHTML={{ __html: this.state.currentMessage.message }}>
        </div>

        <div className="boss-message__meta">
          <div className="boss-message__meta-group">
            <p className="boss-message__meta-text">( Created { moment(this.state.currentMessage.createdAt).format('DD/MM/YYYY')} )</p>
