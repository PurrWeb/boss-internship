...
export default function calculateChartBoundaries(rotaShifts) {
  ...
  if (_.isEmpty(rotaShifts)) {
    ...
    dRotaDate = new RotaDate({
      shiftStartsAt: safeMoment.iso8601Parse(rotaShifts[0].startsAt).toDate(),
    });

    // Adjust offset range everytime we find a shift that's not contained inside it
    rotaShifts.forEach(function(rotaShift) {
      var shiftStartOffset = dRotaDate.getHoursSinceStartOfDay(
        safeMoment.iso8601Parse(rotaShift.startsAt).toDate(),
      );

      var shiftEndOffset = dRotaDate.getHoursSinceStartOfDay(
        safeMoment.iso8601Parse(rotaShift.endsAt).toDate(),
      );
  ...
  var boundaries = {
    start: new Date(
      dRotaDate.startTime.valueOf() + startOffset * MILLISECONDS_PER_HOUR,
    ),
    end: new Date(
      dRotaDate.startTime.valueOf() + endOffset * MILLISECONDS_PER_HOUR,
    ),
  };
  return boundaries;
}
