...
export const renderTooltipTimeHtml = data => {
  ...
  const renderHeader = (group, shiftDates) => {
    ...
    <div class="boss-rotas-time__title">
      <span class="boss-rotas-time__label">
          <span class="boss-rotas-time__pointer" style="background-color:${
            group.color
          }"></span>
          <span class="boss-rotas-time__role">${group.name}</span>
      </span>
      <span> Roated for </span>
      <span class="boss-rotas-time__total">${moment(shiftDates.date).format(
        'HH:mm',
      )}</span>
    </div>
    `;
  ...
  const renderShifts = (shifts, staff) => {
    return shifts.reduce((lines, shift) => {
      const shiftStaff = staff.find(s => s.id === shift.staffMemberId);
      ...
      return (
        ...
        <li class="boss-rotas-time__item">
          <p class="boss-rotas-time__interval">
              <span class="boss-rotas-time__start">${moment(
                shift.startsAt,
              ).format('HH:mm')}</span>
              <span> - </span>
              <span class="boss-rotas-time__end">${moment(shift.endsAt).format(
                'HH:mm',
              )}</span>
          </p>
          <p class="boss-rotas-time__name">${shiftStaff.firstName} ${
          shiftStaff.surname
        }</p>
        </li>
      `
...
export const renderTooltipInfoHtml = data => {
  ...
  function renderLine(groupId) {
    ...
    var isSelected = groupObject && selectedGroupId === groupObject.id;

    let line = groupObject
      ? `
          <li class="boss-rotas-info__item">
            <span class="boss-rotas-info__pointer" style="background-color: ${
              groupObject.color
            }"></span>
            <p class="boss-rotas-info__text"><span>${
              groupObject.name
            }</span>: <span>${shifts.length}</span></p>
          </li>`
      : '';
    ...
    if (isSelected) {
      line = `
              <li class="boss-rotas-info__item">
                ...
                <p class="boss-rotas-info__text"><span><strong>${
                  groupObject.name
                }</span>: <span>${shifts.length}</strong></span></p>
              </li>`;
    }
