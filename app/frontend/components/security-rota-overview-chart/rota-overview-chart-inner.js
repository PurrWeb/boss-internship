...
export default class RotaOverviewChartInner extends Component {
  ...
  highlightControls() {
    ...
    const rotasInfoButtonClass = classNames({
      ...
      'boss-button_state_active': this.state.rotasInfo,
    });
    const rotasTimeButtonClass = classNames({
      'boss-button boss-button_type_small boss-button_role_highlight rota-overview-chart__controls-button': true,
      'boss-button_state_active': this.state.rotasTime,
    });
    if (this.props.chartData.length > 0) {
      return (
        <div className="rota-overview-chart__controls">
          <span className="rota-overview-chart__controls-label">
            Highlight:&nbsp;
          </span>
          <button
  ...
  render() {
    ...
    const tooltipGenerator = this.state.rotasInfo
      ? this.props.tooltipInfoGenerator
      : this.props.tooltipTimeGenerator;
