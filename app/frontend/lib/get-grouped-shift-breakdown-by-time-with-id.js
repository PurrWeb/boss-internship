...
*/
export default function getGroupedShiftBreakdownByTimeWithId(options) {
  var {
    shifts,
    staff,
  ...
  shifts.forEach(function(shift, i) {
    ...
    breakdown.forEach(function(samplingPoint) {
      var startsBeforeOrAtSamplingPoint =
        samplingPoint.date >= safeMoment.iso8601Parse(shift.startsAt).toDate();
      var endsAfterSamplingPoint =
        samplingPoint.date < safeMoment.iso8601Parse(shift.endsAt).toDate();
      var shiftCoversSamplingPoint =
        startsBeforeOrAtSamplingPoint && endsAfterSamplingPoint;
