class FindOrCreateFinanceReport
  def initialize(staff_member:, week:, mark_existing_requiring_update: false)
    @staff_member = staff_member
    @week = week
    @mark_existing_requiring_update = mark_existing_requiring_update
  end
  attr_reader :staff_member, :week, :mark_existing_requiring_update
  ...
  def call
    ...
    if finance_report.present?
     if mark_existing_requiring_update
