class ApiAccessToken
  ...
  def token_key(token)
    ApiAccessToken.token_key(token)
  end
  ...
  def self.find_by_token(token:)
    ...
    if token_data_raw.present?
      ...
      ApiAccessToken.new(token: token, staff_member: staff_member, api_key: api_key)
    end
  end
  ...
  def self.find_by_api_key(api_key:)
    ...
    token_data.map do |token|
      ApiAccessToken.find_by_token(token: token)
    end
  end

  def self.api_key_token_list_key(api_key)
    "api_key_token_list-api_key:#{api_key.key}"
  end

  def api_key_token_list_key(api_key)
    ApiAccessToken.api_key_token_list_key(api_key)
  end
