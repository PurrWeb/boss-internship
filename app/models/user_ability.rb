class UserAbility
  ...
  def initialize(user)
    ...
    if user && user.enabled?
      ...
      can :create, :id_scanner_app_api_keys do
        user.has_effective_access_level?(AccessLevel.admin_access_level)
      end
      ...
      can [:create, :view, :update], SafeChecksPage do |safe_checks_page|
        user.payroll_manager? ||
      ...
      can [:create_holiday_request], StaffMember do |staff_member|
        user.food_ops_manager? ||
        user.payroll_manager? ||
          can_edit_staff_member?(user, staff_member)
      end

      can [:destroy], HolidayRequest do |holiday_request|
        user.food_ops_manager? ||
        user.payroll_manager? ||
