  ...
  describe 'validations' do
    ...
    context 'times are in wrong order' do
      ...
      specify 'should raise error' do
        _break.validate
        expect(_break.errors[:base]).to include("starts_at must be after ends_at")
      end
    end
    ...
    context 'break overlaps existing break' do
      let(:existing_break_clock_in_period) { clock_in_period }
      let(:existing_break_starts_at) { starts_at }
      let(:existing_break_ends_at) { ends_at }
      let(:existing_break) do
        ClockInBreak.create!(
          clock_in_period: existing_break_clock_in_period,
