...
describe ReClockInStaffMemberToNextDay, :clocking do
  ...
  let(:service) do
    ReClockInStaffMemberToNextDay.new
  end
  ...
  context "when ClockInPeriod pass a threshold" do
    context "when clockin" do
      ...
      it "should return a ClockInPeriod" do
        ...
        created_clock_in_period = ClockInPeriod.joins(:clock_in_day).where(clock_in_days: {date: business_date}).first
        expect(created_clock_in_period.present?).to eq(true)
        expect(created_clock_in_period.clock_in_events.last.clock_in?).to eq(true)
      end
    end
  ...
  context "when ClockInPeriod ended" do
    ...
    it "should return a 0 ClockInPeriod" do
      service_result = service.call
      created_clock_in_period = ClockInPeriod.joins(:clock_in_day).where(clock_in_days: {date: business_date}).first
      expect(created_clock_in_period.present?).to eq(false)
    end
  end
