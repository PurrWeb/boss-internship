...
describe GenerateFinanceReportData do
  ...
  let(:owed_hour_create_week) { RotaWeek.new(previous_week.start_date - 1.week) }
  ...
  before do
    travel_to RotaShiftDate.new(previous_week.start_date).start_time + 1.minute do
    ...
    travel_to owed_hour_create_week.start_date + 3.days do
      OwedHour.create!(
        staff_member: staff_member,
        date: owed_hour_date,
        ...
        creator: user,
        note: 'Blah Blah',
        payslip_date: previous_week.start_date
      )
    end
  end
