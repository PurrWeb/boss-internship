...
describe MarkFinanceReportRequiringUpdate do
  ...
  context 'when no finance report exists for week' do
    ...
    specify 'it should create a new finance report marked for update' do
      ...
      expect(created_report.venue_name).to eq(staff_member.master_venue.name)
      expect(created_report.staff_member_name).to eq(staff_member.full_name)
      expect(created_report.pay_rate_description).to eq(staff_member.pay_rate.text_description_short)
