...
RSpec.describe 'DeleteStaffMember service'  do
  ...
  context 'after call' do
    ...
    context 'staff member has holidays in the past' do
      let(:holiday) do
        travel_to 3.weeks.ago do
          week =  RotaWeek.new(RotaShiftDate.to_rota_date(1.week.from_now))
          FactoryGirl.create(
            :holiday,
            staff_member: staff_member,
    ...
    context 'staff member has future holidays' do
      let(:next_week) { RotaWeek.new(RotaShiftDate.to_rota_date(Time.current + 1.week)) }
      let(:holiday) do
        FactoryGirl.create(
          :holiday,
          staff_member: staff_member,
          start_date: next_week.start_date + 1.days,
          end_date: next_week.start_date + 3.days,
          creator: requester
        )
      end
